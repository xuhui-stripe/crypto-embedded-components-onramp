 ...
 *
 * The `limits` object is keyed by currency pair (e.g. "usd.fiat") and then
 * by payment method type. Each entry is an array because a single payment
 * method can have different limits per settlement speed.
 *
 * Example:
...
export interface TransactionLimitsResponse {
  ...
  limits: {
    ...
    [key: string]: { card?: LimitEntry[]; us_bank_account?: LimitEntry[] } | undefined;
  };
}
...
export async function getTransactionLimits(
  ...
  return get(`/v1/crypto/onramp_transaction_limits${query}`, authToken);
}
